...
PMark <- function(cut_p = 0.8, n, ...){
  newdata_dfa <- datf[sample(1:length(datf$sex), n, replace = T), ]
...
loop <- function(...){
  for(i in 10:80){
    newdata <- datf[sample(1:length(data$sex), i, replace = F), ]
    ...
    lr_newdata$sex[lr_newdata$sex == "M"] <- 0
    lr_newdata$sex[lr_newdata$sex == "F"] <- 1
    lr_newdata$sex <- as.numeric(lr_newdata$sex)
...
#Uploading data
#datf <- read.table(file.choose(), header = T, dec = ".") #when prompted, select the MBhum.txt file
datf <- read_csv(here("MBhum.csv"))

#Only complete cases
datf <- na.omit(datf)
colnames(datf) <- c("individual", "sex", "length", "head", "epi")
female <- subset(datf, datf$sex == "F")
male <- subset(datf, datf$sex == "M")
dfa_data <- datf
#re-coding of the sexes for logistic regression: male = 0; female = 1
lr_data <- datf
lr_data$sex <- ifelse(lr_data$sex == "M", 0, 1)
...
boxM(datf[,3:5], datf$sex)
bartlettTests(datf[,3:5], datf[,2])
by(datf[,3:5], datf$sex, cov)
bartlettTests(datf[,4:5], datf[,2])
by(datf[,4:5], datf$sex, cov)
  #multicolin
cor(datf[,3:5])
...
car::vif(lreg) #as a general rule, a result above 5 would be problematic
  #outliers
outlier <- mvoutlier::aq.plot(datf[,3:5])
as.data.frame(cbind(as.character(outlier$outliers), as.character(datf$sex))) #3 female and 1 male outlier
out_data <- datf[-c(2,5,10,76),] #removal of outliers
...
lr_data$lintest_length <- log(datf[,3])*datf[,3]
lr_data$lintest_head <- log(datf[,4])*datf[,4]
lr_data$lintest_epi <- log(datf[,5])*datf[,5]
lintest <- glm(sex ~ length + head + epi + lintest_length + lintest_head + lintest_epi,
               data = lr_data, family = binomial(link = "logit"))
summary(lintest)

#MANOVA
maov <- manova(cbind(length, head, epi) ~ sex, data = datf)
summary(maov)
summary.aov(maov)
...
dfa <- lda(sex ~ length, data = datf, CV = F) #LDF1
dfa <- lda(sex ~ head, data = datf, CV = F) #LDF2
dfa <- lda(sex ~ epi, data = datf, CV = F) #LDF3
dfa <- lda(sex ~ head + epi, data = datf, CV = F) #LDF4
dfa <- lda(sex ~ length + head + epi, data = datf, CV = F) #LDF5
pred_dfa <- predict(dfa, prior = c(0.5,0.5)) #only for CV = F
#with loo cross-validation
dfa_cv <- lda(sex ~ length, data = datf, CV = T)
dfa_cv <- lda(sex ~ head, data = datf, CV = T)
dfa_cv <- lda(sex ~ epi, data = datf, CV = T)
dfa_cv <- lda(sex ~ head + epi, data = datf, CV = T)
dfa_cv <- lda(sex ~ length + head + epi, data = datf, CV = T)

#plot of lda
ldahist(pred_dfa$x, type = "b", g = datf$sex, col = "#eb3434")
dfa_pl <- as.data.frame(cbind(pred_dfa$x, datf$sex))
...
#accuracy
#confusion matrix
tb1 <- table(dfa_cv$class, datf$sex)
...
pred2 <- data.frame(pred2$class, datf$sex)
pred2 <- na.omit(pred2)
...
pred3 <- as.data.frame(cbind(pred3$class, datf$sex))
pred3 <- na.omit(pred3)
...
acc_lreg <- data.frame(pred2_lreg, datf$sex)
...
pred3_lreg <- data.frame(pred3_lreg, datf$sex)
